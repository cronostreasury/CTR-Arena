#!/usr/bin/env node
/**
 * SWEEP Trading Season — Snapshot Builder
 *
 * Runs in GitHub Actions every 5 minutes. Fetches all SWEEP token transfers
 * involving the LP pair from Cronos Explorer API, builds the leaderboard,
 * computes deterministic jackpot rolls, fetches SWEEP balances via RPC,
...
const SWEEP        = '0x8F93de1d32e732Ec37Dc93A373bf8C4aE48392f8';
const LP           = '0x7BFcCe870e7651e577B750F9116E2E8ebC647910';
...
/**
 * Fetch ALL SWEEP Transfer events involving the LP across the entire season window.
 ...
 */
/**
 * Fetch SWEEP LP transfers from chain.
    ...
    const results = await Promise.all(batch.map(async ({ start, end }) => {
      ...
      try {
        const [fromLogs, toLogs] = await Promise.all([
          rpcCall('eth_getLogs', [{ address: SWEEP, fromBlock: fb, toBlock: tb, topics: [TRANSFER_SIG, lpPad, null] }]),
          rpcCall('eth_getLogs', [{ address: SWEEP, fromBlock: fb, toBlock: tb, topics: [TRANSFER_SIG, null, lpPad] }])
        ]);
...
async function fetchBalance(addr) {
  try {
    ...
    const result = await rpcCall('eth_call', [{ to: SWEEP, data }, 'latest']);
...
async function buildWallets(trades) {
  ...
  // Fetch SWEEP balances for all wallets in parallel batches
  const wallets = Object.keys(tm);
  console.log(`  Fetching ${wallets.length} SWEEP balances via RPC...`);
...
async function main() {
  const t0 = Date.now();
  console.log('=== SWEEP Season Snapshot Builder ===');
  ...
  const TRADES_PATH   = path.join(DATA_DIR, 'trades-sweep.json');
  const SNAPSHOT_PATH = path.join(DATA_DIR, 'season-sweep.json');
