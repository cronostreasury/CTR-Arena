...
const LP    = LP_ADDRESS.toLowerCase()
const VAULT = VAULT_WALLET.toLowerCase()
const DEAD  = DEAD_WALLET.toLowerCase()

module.exports = async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*')
  res.setHeader('Cache-Control', 's-maxage=20, stale-while-revalidate=40')
  try {
    const url  = `https://explorer-api.cronos.org/mainnet/api/v1/account/tokentx?contractaddress=${CTR_CONTRACT}&page=1&offset=200&sort=desc&apikey=${API_KEY}`
